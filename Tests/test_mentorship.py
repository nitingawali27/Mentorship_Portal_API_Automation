...
from Utilities.api_url import welcome_url, search_employee_url, employee_details_url
...
class TestAPI:
    ...
    def test_get_message(self):
        ...
        verify_response_key(
            data.get("message"), "Welcome to the Mentorship Program Portal", "message"
        )
    ...
    def test_get_employee_search(self):
        ...
        for employee in data["UserSearchApi"]:
            ...
            if (
                ...
                )  # print instead of logger for simplicity
    ...
    # -----------------------------
    # GET Employee Details Request Test
    # -----------------------------
    def test_get_employee_details(self):
        logger.info("Starting GET Employee Details test...")
        response = self.client.get(employee_details_url)
        data = response.json()
        logger.info(f"GET Employee Details response:\n{json.dumps(data, indent=4)}")
        verify_http_status_code(response, 200)
        verify_json_key_not_null(data, "employeeCode")
        verify_response_contains(response, firstname)
        verify_response_contains(response, lastname)
        verify_response_contains(response, employeecode)
        print("Employee details verified successfully.")
        logger.info("GET Employee Details test completed successfully.")
